...
path_results <- file.path(path_local, "results", sprintf("replica=%i", replica), "dm_bummer")
...
X_knots <- seq(min(X_train, na.rm = TRUE) - 1.25*sd(X_train, na.rm = TRUE),
               ...
               length = n_knots)
params <- list(n_adapt = 40000, n_mcmc = 10000, n_thin = 10,
               likelihood = "dirichlet-multinomial", function_type = "bummer",
               n_chain = 2L, n_cores = 2L)

# Fit
if (!file.exists(file.path(path_results, "mod.rds"))) {
  dm_bummer <- BayesComposition::fit_compositional_data(y = Y_train, X = X_train, params = params,
                                                    progress_directory = path_results,
                                                    progress_file = "progress_file.txt")
  saveRDS(object = dm_bummer, file = file.path(path_results, "mod.rds"))
}
dm_bummer <- readRDS(file = file.path(path_results, "mod.rds"))

# Extract samples
samples <- BayesComposition::extract_compositional_samples(dm_bummer)
...
ip_dm_bummer <- array(dim = c(nrow(res$X), 1L, ncol(res$X)))
for (i in seq_len(ncol(res$X))) ip_dm_bummer[,1L,i] <- res$X[, i]

saveRDS(ip_dm_bummer,
        file = file.path(path_results, "ip_dm_bummer.rds"))

compute_prediction_metrics(x = X_test, draws = ip_dm_bummer)
